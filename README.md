...
- **Reproducibility artifacts** for published calculations — image tags are immutable, so `ghcr.io/material-codes/abinit-base:10.8.3` ships exactly the ABINIT/LibXC binaries that were current at release time
...
```sh
docker pull ghcr.io/material-codes/abinit-base:10.8.3
```

| Tag pattern | Meaning |
|---|---|
| `<version>` (e.g. `10.8.3`) | Pinned to a specific ABINIT release. Immutable. |
| `<version>-N` (e.g. `10.8.3-1`) | Patch revision — ABINIT version unchanged, but a build dependency (LibXC, Debian base) bumped. |
...
```dockerfile
FROM ghcr.io/material-codes/abinit-base:10.8.3
...
```sh
docker build --build-arg ABINIT_VERSION=10.8.3 -t abinit-base:10.8.3 .
```
...
```sh
docker build --platform linux/arm64 --build-arg ABINIT_VERSION=10.8.3 -t abinit-base:10.8.3-arm64 .
```
...
2. Cut a **patch revision tag** of the current ABINIT version, e.g. `v10.8.3-1`. The published image will be `ghcr.io/material-codes/abinit-base:10.8.3-1`. The original `10.8.3` image stays in place (tag immutability), and `latest` moves to the patch.
...
Image tags follow ABINIT's upstream release versioning (e.g. ABINIT release `10.8.3` → image tag `10.8.3`). Patch revisions (LibXC bump, Debian base bump, build-arg default change) get a `-N` suffix.
